# ============================================================
# Lineage tracing — Cre double-labelling quantification (merge across images)
#
...
file_path <- "data/lineage_tracing_Cre_double.xlsx"
...
df2 <- df[1:18,] %>% 
  mutate(
    marker = case_when(
      str_detect(filename, "acta2") ~ "Acta2",
      str_detect(filename, "Col1a1") ~ "Col1a1",
      str_detect(filename, "Pdgfrb") ~ "Pdgfrb"
    ),
    `Merge%` = as.numeric(`Merge%`),RFP = as.numeric(`RFP+`),Merge = as.numeric(Merge)
  )%>%mutate(ISH_RFP=Merge/RFP*100)
...
ggsave(
  filename = "output/2026.06.08_BDL_merge.pdf",
  plot = combined_plot,
  width = 90,
  height = 50,
  units = "mm",
