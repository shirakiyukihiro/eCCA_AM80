...
# Detects Meflin-lineage (tdTomato+) cells in light-sheet .ims volumes within region
# masks. Outputs cell centroids (.npz) and the detection parameters used (.json).
...
"""
Difference-of-Gaussians (DoG) cell detection in light-sheet .ims volumes.

Detects tdTomato+ (Meflin-lineage) cells inside manually drawn region masks and
writes their centroids in micrometres. This is step 1 of the quantitative pipeline;
see README_3D.md.

Implementation notes:
  - the mask JSON stores per-Z polygons under `polygons_um` in world micrometres,
    which are converted to voxel coordinates here;
  - masks are interpolated between annotated Z slices;
  - the Imaris reader is used without a context manager (it does not implement one).
"""
import os, sys, json
import numpy as np
