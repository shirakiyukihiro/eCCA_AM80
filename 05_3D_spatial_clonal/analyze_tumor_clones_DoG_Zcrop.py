#!/usr/bin/env python
# ============================================================
# Clonal cluster analysis (DBSCAN + Monte-Carlo null), Z < 1500 um
...
# Clusters detected cells (DBSCAN, eps=50 um), derives clone candidates against a Monte-Carlo CSR null, and writes per-cluster features.
...
"""
BDC tumor DoG clone analysis - Z < 1500 µm のみ
==================================================

検証で判明した light attenuation を回避するため、
detection が uniform な範囲 (Z < 1500 µm) でクローン解析を再実行。

Output: 元の analysis と並列で比較できる形式
"""
import os, sys, json
import numpy as np
import matplotlib
