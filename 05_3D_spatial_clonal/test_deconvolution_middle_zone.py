#!/usr/bin/env python
# ============================================================
# Deconvolution robustness test — MIDDLE zone (within the uniformly detected zone)
...
#
# Purpose    : Tests whether Richardson-Lucy deconvolution recovers cells that DoG
#              detection misses. A sub-volume centred on the largest cluster
#              (75 cells, Z ~ 1151 um) is deconvolved with an anisotropic Gaussian PSF,
#              and DoG detection is re-run with identical parameters on the original
#              and the deconvolved volume. Supports the statement in the Supplementary
#              Methods that deconvolution recovers ~5% additional cells at mid-depth.
#
# Input      : data/BDC.ims                                    (not distributed; on request)
#              output/BDC_tumor_DoG_Zcrop_clones.npz           (analyze_tumor_clones_DoG_Zcrop.py)
# Output     : output/deconv_test_middle_montage.png
#              output/deconv_test_middle_summary.txt
# ============================================================
...
SUBVOL_HALF_UM = (200, 350, 350)   # half-extent of the test sub-volume, (Z, Y, X) in um

NUM_ITER = 10                      # Richardson-Lucy iterations
PSF_SIGMA_VOX = (1.5, 1.0, 1.0)    # Gaussian PSF sigma in voxels, (Z, Y, X)
                                   # -> with the level-1 voxel size of this dataset
                                   #    (5.22, 2.35, 2.35) um, this is
                                   #    sigma_z = 7.83 um, sigma_xy = 2.35 um
...
OUT_PNG = f'{OUT_DIR}/deconv_test_middle_montage.png'
OUT_TXT = f'{OUT_DIR}/deconv_test_middle_summary.txt'
...
def gaussian_psf_3d(sigma_vox):
    ...
    shape = tuple(max(3, int(6 * s + 1) | 1) for s in sig)   # odd, ~6 sigma
...
# ============================================================
# 1. Locate the largest cluster (target for the test)
# ============================================================
print('=' * 70)
print('  Deconvolution robustness test — MIDDLE zone')
print('=' * 70)
...
top_cid, n_top = counts.most_common(1)[0]
center_um = centroids[labels == top_cid].mean(axis=0)
print(f'  Largest cluster: id={top_cid}, n={n_top} cells')
print(f'  Centroid: Z={center_um[0]:.0f}  Y={center_um[1]:.0f}  X={center_um[2]:.0f} um')

# ============================================================
# 2. Read the sub-volume
# ============================================================
print('\n[2] Reading sub-volume from .ims ...')
...
print(f'  Intensity:  {sub.min()} - {sub.max()}')

# ============================================================
# 3-5. Detect -> deconvolve -> re-detect
# ============================================================
print('\n[3] DoG detection on the ORIGINAL sub-volume ...')
...
print(f'\n[4] Richardson-Lucy deconvolution ({NUM_ITER} iterations) ...')
psf = gaussian_psf_3d(PSF_SIGMA_VOX)
print(f'  PSF shape {psf.shape}, sigma_vox {PSF_SIGMA_VOX}')
...
print('\n[5] DoG detection on the DECONVOLVED sub-volume ...')
...
for row, (vol, peaks, name, colour) in enumerate([
        (sub, peaks_orig, 'ORIGINAL', 'cyan'),
        (deconv_int, peaks_deconv, 'DECONVOLVED', 'yellow')]):
...
fig.suptitle(f'Deconvolution test — MIDDLE zone (largest cluster, {n_top} cells, '
             f'Z = {center_um[0]:.0f} um)\n'
             f'Gaussian PSF sigma_vox = {PSF_SIGMA_VOX}, Richardson-Lucy {NUM_ITER} iter | '
...
with open(OUT_TXT, 'w') as f:
    f.write('Deconvolution robustness test - MIDDLE zone\n')
    f.write('=' * 50 + '\n\n')
    f.write(f'Target: largest cluster (id {top_cid}, {n_top} cells)\n')
    ...
    f.write('Reported in the Supplementary Methods as ~5% additional cells recovered\n')
    f.write('at mid-depth, i.e. insufficient to alter the primary results.\n')

print(f'\nFiles written:\n  {OUT_PNG}\n  {OUT_TXT}')
print('=' * 70)
print(f'  MIDDLE zone: {len(peaks_orig)} -> {len(peaks_deconv)} cells ({ratio:.2f}x)')
print('=' * 70)
